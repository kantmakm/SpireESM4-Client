<cfcomponent displayname="MyCFCTest" extends="mxunit.framework.TestCase">
	...
	<cffunction name="setUp" returntype="void" access="public">		
		...
		<cfquery datasource="#variables.requestObject.getVar('dsn')#">
			INSERT INTO events ( id,name,title,location,description,startdate,enddate,changedby,siteid,onhomepage)
			VALUES (
    ...
    <cffunction name="testGettingEvents">
		<cfset var lcl = structNew()>
    	<cfset var itm = variables.controller.getEventsModel('Events')>
        <cfset var count = "">
    ...
    <cffunction name="testShowHTML">
		...
		<!--- event listing --->
		<cfset data.pageing="1000">
    	<cfset loadController(data = data)>
		<cfset variables.controller = variables.controller.type(module="events", moduleaction="type")>
		<cfset html = variables.controller.showHTML(module="events", moduleaction="type")>
        <cfset asserttrue(condition = refind('<ul class="bullets">.*</ul>',html),message="did not find matching ul elements")>
		...
		<!--- event item --->
		<cfset data.view = "eventdetail"> 
		<cfset itm = variables.controller.getEventsModel("events")>
		<cfset data.eventInfo = itm.getEvent(id = variables.eventid)>
		
		<cfset data.itemid = variables.eventid>
    	<cfset loadController(data = data)>
		<cfset variables.controller = variables.controller.item(module="events", moduleaction="eventdetail")>
		<cfset html = variables.controller.showHTML(module="events", moduleaction="eventdetail")>
    ...
    <!--- <cffunction name="testRegistrationForm">
		...
		<cfset lcl.itm = variables.controller.getEventsModel('Events')>
